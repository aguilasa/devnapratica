 ...
 * Trabalho Final
 */
public class ImportShoppingListImpl {
	...
	public ImportShoppingListImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportShoppingListInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_SHOPPING_LIST, //
	...
	*/
	public ImportShoppingListOutput importShoppingList(ImportShoppingListInput input, long timeout) {
		...
		ImportShoppingListOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportShoppingListOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportShoppingListOutput> importShoppingListRequest(ImportShoppingListInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportShoppingListOutput.class);
	}
