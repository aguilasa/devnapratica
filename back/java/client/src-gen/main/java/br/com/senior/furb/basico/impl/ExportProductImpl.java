 ...
 * Trabalho Final
 */
public class ExportProductImpl {
	...
	public ExportProductImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportProductInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_PRODUCT, //
	...
	*/
	public ExportProductOutput exportProduct(ExportProductInput input, long timeout) {
		...
		ExportProductOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportProductOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportProductOutput> exportProductRequest(ExportProductInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportProductOutput.class);
	}
