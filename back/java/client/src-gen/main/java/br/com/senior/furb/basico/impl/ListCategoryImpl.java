...
public class ListCategoryImpl {
	...
	public ListCategoryImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Category.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_CATEGORY, //
	...
	*/
	public Category.PagedResults listCategory(Category.PageRequest input, long timeout) {
		...
		Category.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Category.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Category.PagedResults> listCategoryRequest(Category.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Category.PagedResults.class);
	}
