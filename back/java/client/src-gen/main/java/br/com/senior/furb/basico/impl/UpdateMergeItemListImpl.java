 ...
 * Trabalho Final
 */
public class UpdateMergeItemListImpl {
	...
	public UpdateMergeItemListImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ItemList input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_ITEM_LIST, //
	...
	*/
	public ItemList updateMergeItemList(ItemList input, long timeout) {
		...
		ItemList output = DtoJsonConverter.toDTO(resultMessage.getContent(), ItemList.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ItemList> updateMergeItemListRequest(ItemList input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ItemList.class);
	}
