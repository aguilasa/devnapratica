 ...
 * Trabalho Final
 */
public class CreateMergeCategoryImpl {
	...
	public CreateMergeCategoryImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Category input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_CATEGORY, //
	...
	*/
	public Category createMergeCategory(Category input, long timeout) {
		...
		Category output = DtoJsonConverter.toDTO(resultMessage.getContent(), Category.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Category> createMergeCategoryRequest(Category input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Category.class);
	}
