 ...
 * Trabalho Final
 */
public class CreateMergeProductImpl {
	...
	public CreateMergeProductImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Product input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.CREATE_MERGE_PRODUCT, //
	...
	*/
	public Product createMergeProduct(Product input, long timeout) {
		...
		Product output = DtoJsonConverter.toDTO(resultMessage.getContent(), Product.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Product> createMergeProductRequest(Product input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Product.class);
	}
