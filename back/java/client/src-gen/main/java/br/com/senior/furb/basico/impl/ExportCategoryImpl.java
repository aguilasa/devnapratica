 ...
 * Trabalho Final
 */
public class ExportCategoryImpl {
	...
	public ExportCategoryImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportCategoryInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_CATEGORY, //
	...
	*/
	public ExportCategoryOutput exportCategory(ExportCategoryInput input, long timeout) {
		...
		ExportCategoryOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportCategoryOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportCategoryOutput> exportCategoryRequest(ExportCategoryInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportCategoryOutput.class);
	}
