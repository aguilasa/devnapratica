 ...
 * Trabalho Final
 */
public class ExportShoppingListImpl {
	...
	public ExportShoppingListImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportShoppingListInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_SHOPPING_LIST, //
	...
	*/
	public ExportShoppingListOutput exportShoppingList(ExportShoppingListInput input, long timeout) {
		...
		ExportShoppingListOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportShoppingListOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportShoppingListOutput> exportShoppingListRequest(ExportShoppingListInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportShoppingListOutput.class);
	}
