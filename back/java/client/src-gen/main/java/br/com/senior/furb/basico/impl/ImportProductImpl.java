 ...
 * Trabalho Final
 */
public class ImportProductImpl {
	...
	public ImportProductImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportProductInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_PRODUCT, //
	...
	*/
	public ImportProductOutput importProduct(ImportProductInput input, long timeout) {
		...
		ImportProductOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportProductOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportProductOutput> importProductRequest(ImportProductInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportProductOutput.class);
	}
