...
public class ImportCategoryImpl {
	...
	public ImportCategoryImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportCategoryInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.IMPORT_CATEGORY, //
	...
	*/
	public ImportCategoryOutput importCategory(ImportCategoryInput input, long timeout) {
		...
		ImportCategoryOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportCategoryOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ImportCategoryOutput> importCategoryRequest(ImportCategoryInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportCategoryOutput.class);
	}
