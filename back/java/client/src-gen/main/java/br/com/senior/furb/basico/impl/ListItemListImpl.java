...
public class ListItemListImpl {
	...
	public ListItemListImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ItemList.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_ITEM_LIST, //
	...
	*/
	public ItemList.PagedResults listItemList(ItemList.PageRequest input, long timeout) {
		...
		ItemList.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), ItemList.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ItemList.PagedResults> listItemListRequest(ItemList.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ItemList.PagedResults.class);
	}
